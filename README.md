...
A infraestrutura de execução está configurada: aplicação Spring Boot, PostgreSQL, Redis, Flyway, Swagger e Docker Compose. O domínio contém modelos abstratos e contratos de casos de uso em Java puro, organizados por área de negócio.

Ainda não há endpoints de negócio, implementações concretas dos casos de uso ou migrations SQL das tabelas do modelo. A configuração de cache está disponível, mas nenhuma funcionalidade de negócio utiliza cache automaticamente.
...
As versões e dependências do backend estão no [pom.xml](pom.xml).
...
O Compose constrói a imagem `phcauto-backend:local` e inicia backend, PostgreSQL e Redis. O backend aguarda os healthchecks das dependências antes de iniciar. A imagem possui etapas separadas de build e execução, usa Java 17 e executa com usuário sem privilégios de root.
...
```bash
POSTGRES_PORT=15432 REDIS_PORT=16379 docker compose up -d --wait postgres redis
POSTGRES_PORT=15432 REDIS_PORT=16379 ./mvnw spring-boot:run
```
...
Consulte os [contratos de domínio](docs/domain.md) e o [modelo DBML](docs/modelo-inicial.dbml). As regras descritas nesses documentos ainda precisam ser implementadas; os contratos não as executam por si só.

## Configuração

A configuração do Spring está em [application.yaml](src/main/resources/application.yaml).
...
| POSTGRES_PORT | 5432 | Porta publicada no host e usada pelo Java local |
...
PostgreSQL utiliza o volume nomeado `postgres_data`, e Redis utiliza `redis_data` com persistência AOF. Os volumes preservam os dados ao recriar os contêineres. **`docker compose down -v` remove os volumes e apaga esses dados.**

O Flyway está habilitado e executa scripts de `src/main/resources/db/migration` ao iniciar o backend. Use nomes como:
...
Esses nomes são exemplos; os scripts ainda não foram criados. O Hibernate usa `ddl-auto: validate`, deixando a evolução do esquema para o Flyway. Não altere migrations já aplicadas em ambientes compartilhados: crie uma nova versão.

## Cache

[RedisConfig.java](src/main/java/repasse/phcauto/backend/infra/config/RedisConfig.java) configura Spring Cache com serialização JSON via Jackson 3, chaves em texto, expiração e sem armazenamento de valores nulos.
...
O teste de contexto requer PostgreSQL acessível. Para executar os testes com as portas padrão:
...
```bash
POSTGRES_PORT=15432 REDIS_PORT=16379 docker compose up -d --wait postgres redis
POSTGRES_PORT=15432 REDIS_PORT=16379 REDIS_INTEGRATION_TEST=true ./mvnw -B -ntp test
```
...
As orientações de arquitetura e manutenção estão em [AGENTS.md](AGENTS.md). Atualize a documentação ao alterar a execução, os contratos ou as configurações do projeto.
