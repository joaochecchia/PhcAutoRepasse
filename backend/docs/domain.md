...
A infraestrutura agora possui entidades JPA, repositories de leitura/escrita e a migration SQL inicial; consulte [persistence.md](persistence.md). Os contratos legados continuam abstratos; o cadastro local é a primeira implementação concreta em `usuarios/internal/core`, com gateways e endpoint próprios. Autenticação e cobrança permanecem pendentes. Os enums expressam os valores permitidos no modelo; não constituem máquinas de estado. A implementação de regras deverá continuar independente dos frameworks, com adaptadores técnicos na infraestrutura.
